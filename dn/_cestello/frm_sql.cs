...
namespace dn_client {
  public partial class frm_sql : Form {
    ...
    //protected string _conn_string = "server=127.0.0.1;uid=root;database=test_db;password=Molina32!;";
    //protected string _provider = "MySql.Data.MySqlClient";

    protected sql_conn _conn = null;

    public frm_sql(core c) {
    ...
    protected void reload_conns() {
      db_provider conn = null;
      try {
        conn = db_provider.create_provider(_c.config.get_conn(_c.config.get_var("lib-vars.db-connection").value));
        foreach (DataRow dr in conn.dt_table(_c.parse_query("sql-client.db-conns")).Rows) {
          db_conn dc = new db_conn(dr);
          cmb_conns.Items.Add(new ComboboxItem() { Text = dc.conn_name + " - " + dc.type.ToString(), Value = dc });
        }
        cmb_conns.SelectedIndex = -1;
      } catch (Exception ex) {
    ...
    protected db_conn sel_conn() {
      if (cmb_conns.SelectedItem == null) return null;
      return (db_conn)((ComboboxItem)cmb_conns.SelectedItem).Value;
    }
  ...
  <tr><th>F5</th><td>Esegui istruzione selezionata o intero script.</td></tr>
    ...
    protected void exec(string sql) {
      try {
        ...
        db_conn dc = sel_conn();
        if (dc == null) throw new Exception("non hai selezionato un database di esecuzione!");
        ...
        foreach (TSqlBatch batch in tsql_fragment.Batches) {
          foreach (TSqlStatement s in batch.Statements) {
            ...
            if (_conn == null) { _conn = new sql_conn(dc.provider, dc.conn_string); }
    ...
    private void frm_sql_FormClosing(object sender, FormClosingEventArgs e) {
      if (!force_close) {
        e.Cancel = true;
        this.Hide();
        this.closed = true;
